// DictFlow — offline voice dictation for Windows (100% local).
...
//
// Release builds are a GUI app (no extra CMD/console window). `tauri dev`
...
struct Settings {
    ...
    language: String,
    auto_paste: bool,
    /// Cleanup level: "off" | "light" (filler words only) | "full"
    /// (fillers + punctuation tidy).
...
/// Hide the console window for child processes (cmd, whisper-cli). Without
/// this, a GUI parent still flashes a CMD box for console-subsystem children.
...
fn open_mic_settings() -> Result<(), String> {
    ...
    cmd.args(["/C", "start", "", "ms-settings:privacy-microphone"]);
